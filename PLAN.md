...
| `app/` (07, 13) | 🟡 M6 client · M5 in progress | theme + router + seams + `features/auth` + `features/devices` (7 screens), 85 tests green · **M5**: `features/onboarding` S0.0/S0.05/S0.1/S0.3/S0.4/**S0.5**/**S0.5b**/S0.6a/S0.6a1/S0.6b/S0.8 (65 tests) · `features/home` S1/S1.1/S1.2/S1.3/S1.4 + S0.7 checklist · `features/ledger` S3/S3.1/S4/**S4.1** · `features/entry` S2/S2.1/**S2.2**/S2.3/**S2.5** · `features/lock` S15/S15.1/S15.3 · `features/settings` S13 — and the **shell now mounts them** (`main.dart`: lock + auto-lock timers + locale/appearance + scope-per-tab). **Push lane green 12 Sep**, app package **286 passed / 1 skipped** (the skip is `F1-07-59`'s drawings posting, blocked on the engine — see M5). See `.claude/lane-reports/M5-*.json` |
