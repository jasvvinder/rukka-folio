...
| `app/` (07, 13) | 🟡 M6 client · M5 in progress | theme + router + seams + `features/auth` + `features/devices` (7 screens), 85 tests green · **M5**: `features/onboarding` S0.0/S0.05/S0.1/S0.3/S0.4 (`F1-07-39/40/41`, `F1-07-16`, `F1-07-48`) + S0.6a/S0.6a1/S0.6b (`F1-07-51`, `F1-07-45`, `F1-13-15`, `F1-09c-1`; 41 tests in `test/features/onboarding`), `features/home` S1 + S1.1 (`F1-07-49/50`, wired into `main.dart` so Home is no longer a placeholder) + S1.2/S1.3 scope switcher + S1.4 rebuilding (`F1-07-52/53`, `F1-07-38`; 30 tests in `test/features/home`), `features/ledger` S3 + S3.1 + S4 (`F1-07-42/43/44`, 19/19), `features/entry` S2 + S2.1 + S2.3 (`F1-07-17`, `F1-07-54/55/56`; 19 tests in `test/features/entry`, `entryScreen` wired as `entryRoot`) — **all seven lanes gated green on the push lane 10 Sep** (app 202 tests). See `.claude/lane-reports/M5-{U1a,U1b,U1c,U2a,U2b,U2c,U3a}.json` |
...
- ⬜ **`A-09c-1` seed test** in `app/test/shared/ledger` — covers the shared-business seed U1c added to `createBook` (untested there, wrong directory) **and** the rest of ADR 2026-09-09c §1 (`Business Cash A/c`, `Sales A/c`, the shop/trade tree), which `createBook` still does not seed. No bank is seeded, as ADR 09d §1 requires
- ⬜ **S0.6b wiring** (⚠️ SPEC at `onboarding_routes.dart:73`): the route mounts S0.6b with `rows: const []` and S0.6a1 with `yourName: ''` — the book is not created in the routes because the committing step (07 §3.1 step 8 / S0.7) has no screen, and S0.4's name is still not carried forward (U1b's gap). U1f owns both: create the book from the S0.6a1 owners and pass the seeded chart in
...
- ⬜ **Scope does not persist per tab** (from U2b, 13 §2.2 "scope persists per tab, defaults to last used"): `HomeScopeController` lives for the screen's lifetime only. Seam wanted in the shell (`app/lib/shared/`, not the lane's directory): one controller or persisted `Scope` per tab, handed to `HomeScreen(scopeController:)` — the parameter already exists
...
- ⬜ S2.2 date · S2.5 drawings confirmation
