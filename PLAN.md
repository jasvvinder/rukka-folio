...
|---|---|---|
| `core_ledger` (02) | ✅ M1 | suite A: 111 ids + golden replay of 8 worked-example books (content_hash unchanged since M1) · **`A-09b-4` drawings landed 12 Sep, 159/159** |
...
| `app/` (07, 13) | 🟡 M6 client · M5 in progress | theme + router + seams + `features/auth` + `features/devices` (7 screens), 85 tests green · **M5**: `features/onboarding` S0.0/S0.05/S0.1/S0.3/S0.4/S0.5/S0.5b/S0.6a/S0.6a1/S0.6b/**S0.6d**/**S0.6e**/**S0.6f**/**S0.6g**/**S0.6h**/**S0.6i**/S0.8 (98 tests) — **every purpose-card branch but the S0.6c loop is now built** · `features/home` S1/S1.1/S1.2/S1.3/S1.4 + S0.7 checklist · `features/ledger` S3/S3.1/**S4 + FY switcher & computed b/f**/S4.1 · `features/entry` S2/S2.1/**S2.2**/S2.3/**S2.5** · `features/lock` S15/S15.1/S15.3 · `features/settings` S13 · **`features/menu` S8** · **`features/reports` S8.1** · **`shared/widgets` S12.5 read-only / book-full pattern** — and the **shell now mounts them** (`main.dart`: lock + auto-lock timers + locale/appearance + scope-per-tab, and **all four bottom-bar tabs are now real screens** — Menu was the last placeholder). **Push lane green 12 Sep (twice — second gate after U1e + U3d, only `dart format` to fix)**, app package **339 passed / 0 skipped before this session's two lanes** — `F1-07-59`'s drawings posting un-skipped once the engine blocker was cleared (U2d-engine). See `.claude/lane-reports/M5-*.json` |
| Traceability | ✅ | `check_coverage --strict` **green** and blocking in `ci.sh`: **662 tests · 460 ids declared · 552 ids named · 332 🔒 lines, 0 unmarked · 0 orphans · 0 warnings** (12 Sep). `C-05a-7` was an orphan until this session marked it on its owning 🔒 line, `06 §98` (foreground inactivity lock, ADR 2026-09-05 §7). 92 lines carry planned ` @M<n>` markers (ADR 2026-09-08) that fail once their milestone lands |
...
7. **Rule the entry kind for "capital introduced"** — `Verbs.moneyIn` carries the exact twin of the drawings blocker just fixed: `from` accepts any `equitySystem` while `checkShape`'s `moneyIn` credits admit `income|party|advance` only. It bites when the 8th S3.1 quick-add tile (ADR 2026-09-09b Open) is built as `Dr money · Cr Opening Balance/Capital`. Decide `money_in` vs `adjustment`; if `money_in`, the fix mirrors U2d-engine. Routine once ruled — **opus tier, not fable**.
...
- ⬜ **S0.6c** *Add another business?* — the last onboarding gap, and the **My businesses** card loops through it (07 §3.1.1). Not a repeat screen: it makes `OnboardingFlow` hold a list of businesses and loop back to S0.6a, so it is flow surgery on a file with 98 green tests over it — `lane-ui-hard`, its own session
