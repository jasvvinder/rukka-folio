...
| `data` (03 client) | ✅ M2 · 🟡 M8/M9 reads | suite E client half: Drift + SQLCipher **schema v3** (v3 = `close_progress_local`, device-local, in neither layer — `E-03-46`, ⚠️ SPEC 03 §3.2), mirror/outbox, Recompute, corruption path · **17 Sep:** `structural_reader` (`E-03-35…45`), pure `partnerPositions` (`E-02-1…10`) |
...
| **B — people** | 14–20 Sep | ✅ **S2/S3/S4** server · ✅ **Y2/W3** transport · ✅ **U4** members+ceremony · ✅ **U6** inbox · ✅ **W2** S0.9 · ✅ **U5a–g** family money (S5, S5.5, S8.3, S14/S14.2, S2.3, S1 in-transit) · 🟡 **CL1/CL2** close (S10 + S10.5 landed) → remaining: **S5.1** (⛔ ruling) · **S14.1** · S10.1–S10.4 · **U7** recovery screens | two phones sync (⛔ blocked on the `CryptoGuard` identity accessor); **TestFlight to the owner's family** |
...
### M9 Close 🟡 (H 8–9, H2b) — opened 17 Sep
...
- ⬜ Home **Close card** (07 §13 bullet 1 → `ClosePaths`) · ⬜ S10.1 family status · S10.2 summary card · S10.3 late arrivals · ⬜ S10.4 year close + FY switcher + certified vectors · structural quorum flow
