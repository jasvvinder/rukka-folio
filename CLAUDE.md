...
## Session economy 🔒 (owner-directed, 7 Sep 2026; restructured 8 Sep 2026; tiers + throughput 12 Sep 2026; ceiling + review loop 21 Sep 2026; 5-slice cap + xhigh + dated overrides 24 Sep 2026) ⟦tests: n/a — process rule, not behaviour⟧
- **Start with `PLAN.md`** §0 + the current phase; not CHANGELOG, not whole specs. Read spec *sections*: `grep -n "^## \|^### " docs/<n>.md` → `sed -n 'a,bp'`.
- **Milestone work runs as `/lane`:** one orchestrator session that holds PLAN rows and lane reports and nothing else; lanes are subagents owning **disjoint directories**. `/lane` runs at most **5** lanes per run and then **stops** (ADR 2026-09-24 §1, was 3 under ADR 2026-09-21 §3). **`/cycle` is the fuller loop** — build → read-only review → adversarial verify → bounded repair — and also caps at 5 slices.
- **A day has a ceiling 🔒 (owner-directed, 21 Sep 2026; ADR 2026-09-21 §1, §4).** This replaces *Fill the session* (ADR 2026-09-12b §6), which gave throughput a floor and no ceiling: 17–19 Sep burned **11.8 M — 45 % of all-time spend — in three days**. A session now runs round after round until **today's** ceiling (`budget.daily_tokens` in `.claude/rf.config.json`, 1.2 M ≈ weekly/6 — or that date's entry in `budget.daily_overrides`, owner-set only; 26–27 Sep 2026 = 20 M, ADR 2026-09-24 §3), then `/close`. 🔒 **The weekly quota is owner-set and not machine-readable** — `wf-spend.sh` reports spend and does not rule on it (ADR 2026-09-13e); read `/usage` and write the number into `budget.weekly_tokens`. Disjointness is still per *run*, never per session: lanes inside one round must own disjoint directories, but the next round may take the directories the last one released. ⟦tests: n/a — process rule, not behaviour⟧
- **Nothing is committable until it has been reviewed and the findings verified 🔒 (owner-directed, 21 Sep 2026; ADR 2026-09-21 §5–§7) ⟦tests: n/a — process rule, not behaviour⟧.** `lane-review` (opus · xhigh, **read-only** — no Write, no Edit) reviews each slice: **test-honesty first** (would this test still pass if the production seam returned empty? — S6 approvals ran on a fake until 18 Sep), then spec conformance, security, 🔒/traceability, invariants. Every finding is then put to verifiers **prompted to refute it**, defaulting to refuted when uncertain — three lenses at xhigh on `core_*`, `sync_engine` and `server/supabase/{migrations,functions}`, one at medium elsewhere (ADR 2026-09-24 §2), none on what a deterministic checker already owns. Survivors go back to the lane that owns the directory, **bounded at two repair rounds**; anything still open, or disputed with evidence, becomes an owner desk item rather than a third round. This is the structural form of rule 11 — a model cannot reliably self-refute, which is why that rule needed writing.
...
- Build: `/cycle <keys>` (build → review → verify → repair; ≤5 slices) · `/lane <keys>` (build only, ≤5 lanes) · `/gate [lane]` (separate run, always) · `/close` (plan → changelog → clear)
- Position / budget: **`.claude/bin/board.sh`** (where the build stands, your desk, spend vs ceilings; `--line` for a statusline; printed automatically at session start) · `.claude/bin/rf-state.py` (writes `.claude/state.json`) · `.claude/bin/wf-spend.sh` (week's spend; `--all`) · `.claude/bin/lane-status.sh` (which lanes landed, which are part-way) · ceilings live in `.claude/rf.config.json`
