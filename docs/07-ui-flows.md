...
**Companions:** 02-ledger-rules.md (every posting and rule referenced here), 04-crypto.md (ceremony, recovery), 06-auth-devices.md (states, roles). Strings live in 01-glossary.md — this document uses English keys; every string ships in EN + ਪੰਜਾਬੀ + हिन्दी (CLAUDE.md rule 8).
...
- **[+]** — opens the entry screen (§5). No long-press repeat — §5's *nothing is pre-selected* rule stands (ADR 2026-09-05f §C).
...
### 3.1 Fresh signup 🔒 ⟦tests: F1-07-15 @M5, F1-07-39, F1-07-40, F1-07-41, F1-07-62⟧
...
3. *"What will you use this for?"* — **five** illustrated cards 🔒 (owner-added 31 Aug 2026): **Myself · My shop · My businesses · My family · Our trust** — the fifth carrying a subtitle the others do not need: *gurudwara, temple, society or registered trust*, because "trust" alone is abstract and a mandir or sabha committee member must see themselves in it. The list is examples, not an exhaustive set; the underlying tenant type is the generic `organization`.. Layout is 2 × 2 with the trust card **full width beneath**, since five does not divide into a grid and the trust label is the longest. Adding it here fixes a real discovery problem: a treasurer would otherwise have had to pick a wrong card and then find *Add organization* in Menu.
   🔒 **This card is the only one that sets `tenant.type = organization`**, which carries consequences beyond seeded accounts: trustee role labels, gollak created as a `cash_collection` account, and **denomination counting mandatory on every cash account** (02 §8.2). Seeds: **Cash (the ordinary cash-in-hand A/c every ledger has)**, Gollak Cash, Donation Income, Langar Expense, Building Repair, Honorarium — **no bank account** (ADR 2026-09-09d §2: a bank is added, not seeded, in every book type). ⟦tests: A-09d-2⟧ (seeds books + category tree only; changeable later; no lock-in copy). 🔒 The gollak and the Cash A/c are **different accounts**: counted money leaves the gollak only by deposit into the Cash A/c or the trust's bank — in one go or in parts, the gollak's balance always showing what is still in the box — and expenses are never paid straight from it (02 §8.2, owner-ruled 2–3 Sep 2026).
4. Name (+ optional photo — shown in approvals & ceremonies, worth asking here). ⟦tests: F1-07-48⟧
5. **Keeping your books safe 🔒 (one onboarding step, owner-directed 31 Aug 2026)** — backup is configured **here, at signup**, not left in Settings, because backup found later mostly never happens. One screen, three items: (a) *Your key is kept in iCloud Keychain* — **on**, stated rather than asked, with the plain line that Apple cannot read it, and one more: *"A phone backup does not carry your books — your key comes back with your Apple account, your books come back from the server"* (ADR 2026-09-05c §8, ADR 2026-09-05f §G); (b) *Automatic backup* — **on**, showing the destination, with the readable-copy disclosure in one prominent sentence and a one-tap off (04 §7.6); (c) the **recovery sheet** action below. If iCloud Keychain is unavailable or disabled on the device, the screen says so plainly and makes the sheet the primary action instead. ⟦tests: F1-07-71, F1-07-72⟧
6. **Recovery sheet moment** (04 §7.4): explain in one screen why there is no "forgot password" (*"Your data is locked so well that even we can't open it"*), generate PDF, share/print, then the **verified-storage nag** badge lives on Menu until they scan the printed sheet back. ⟦tests: F1-07-73⟧
7. Opening balances wizard (02 §4) — skippable, resumable from Home's setup card (S0.7). ⟦tests: F1-07-57⟧
...
Follows the scenario table in 06 §5 verbatim. Key screens: **Link with old phone** (camera + *Show my code* on the old device), **Ask your guardians** (live k-of-n progress: `Sunita ✓ · Rajesh …pending · Bauji —`, each row shows *called them?* hint), **Scan your paper sheet**, and the honest last screen: *"Shared books can be restored by your family after they verify you again; your old personal book cannot be recovered."* No euphemisms. **Two rules from ADR 2026-09-05f §B/§G:** before its certificate is verified a new device names **nothing** about the family (no "we found your family" copy — ADR 2026-09-05d §2); and when the user still has an active device, the fork screen says *"Linking is instant — recovery takes 24 hours"* and the existing devices show **S11.9 Recovery in progress — Cancel** (ADR 2026-09-05d §1).
...
```
┌──────────────────────────────────────┐
│ ⌄ Sharma Family        ✓  ⟳         │  scope chip · integrity · sync (no bell — ADR 2026-09-05f §F)
│                                      │
│  TOTAL MONEY YOU HAVE   ₹2,41,500  │  ← hero (owner-approved)
│  SBI ₹1,38,210 · Cash ₹18,240 ...   │
│  ── ✓ BOOKS BALANCED ─────────────  │  ← verification card
│  Books balanced · difference nil     │  ← plain words on Home; Dr/Cr live in the TB
│  POSITION (this scope)               │
│  Cash in hand            ₹ 12,450    │
│  SBI Saving              ₹ 1,08,210  │
│  HDFC CC              −  ₹ 22,915    │
...
- **Books-balanced verification card 🔒 (owner-approved):** replaces the bare ✓/✗ integrity light — shows the status pill and the difference **in plain words** (*"Books balanced · difference nil"*; law 2 — Home is a consumer surface), tapping through to the full trial balance (**S8.2** report viewer) where Dr and Cr totals are shown (ADR 2026-09-05f §F). While a book is rebuilding (local corruption, Recompute on upgrade, `store_epoch` re-pull — ADR 2026-09-05c §3/§6) the card is replaced by **S1.4 Book incomplete — rebuilding** with the determinate loader; `integrity_ok` gates it. In Everything scope each book card carries its own **provisional** badge when that book has an author gap. In this product it is a genuine trust artefact, not decoration: at close, every member's device recomputes the same vector independently (02 §8).
...
🔒 The chip row shows the three most-used money accounts plus **+ More** for the full list; it is **absent entirely** when no money account is involved (milk on khata: `Dr Milk Expense · Cr Vardhman Dairy`). **No chip is pre-selected** — the single-screen rule below governs (ADR 2026-09-05f §C).
3. **Slot B** (per the table above): **a full screen, not a sheet** — recents and favourites fill it, search at the bottom; **inline create** per 02 §1.2 (*"+ Create 'Langar' A/C"*, class inferred from slot, the one two-chip question only when the slot is ambiguous). ⟦tests: F1-07-55⟧
4. **Date chip — `Today`** 🔒: tap → calendar; any date in an open period selectable (backdating); **future dates disabled** (tooltip: *"Use a recurring entry for future dates"*); locked dates shown 🔒-greyed — tapping one explains the lock and offers ***Fix an old entry*** (reversal flow, 02 §5). Never a dead error. ⟦tests: F1-07-58⟧
...
**Adjustment** lives behind its own doors (S2.4: one door each — ADR 2026-09-03b) and shows only its wizards (02 §2 row 6), never a freeform journal screen; **Move money is the pill's fifth position, not a chooser row** (ADR 2026-09-05f §C). Inter-book transfer: choosing a destination in another book switches the flow to §10. **Book full** (`rejected:quota`, ADR 2026-09-05b §7): Save is blocked with the S12.5 sheet pattern pointing to S12.1, the draft is preserved, reading and exports keep working.

**Owner's drawings 🔒 (ratified 2 Sep 2026, ADR 2026-09-02 — design B5, canvases 2/12):** in a business book, money the owner takes for himself posts to **Drawings (equity)**, never to an expense category. When the counterpart is the owner, a one-line confirmation states it plainly — *"Money taken for yourself — recorded as a drawing, not a business expense."* The sheet exists so a shopkeeper never inflates expenses by paying himself; the posting itself is 02's equity rule. ⟦tests: F1-07-59⟧
...
- 🔒 **Screenshots and screen recording are blocked (owner-ruled 5 Sep 2026, ADR 2026-09-05 §4).** Android `FLAG_SECURE` over the whole app; on iOS, which has no equivalent, the cover also drops over amounts while screen capture or recording is active. Every legitimate reason to capture a screen already has a door — share a statement or receipt as **PDF or over WhatsApp** (§14) — so nothing is lost, and a balance in an auto-uploading photo roll is exactly the leak we exist to stop. **The block has a consequence and a door** (ADR 2026-09-05f §G): when a capture is attempted the app says *"Screenshots are off in this app to protect your books. To show someone a page, share it as a PDF"* with Share as PDF and Help. It also covers **Show my code** (S9.2) — the other phone's camera is unaffected.
- **The lock family has more than one state (ADR 2026-09-05f §B):** **S15.4 Device suspended** — the server asserted a revocation without a signed record: read-only with a persistent banner, sync stopped, nothing wiped, *Retry* (ADR 2026-09-05b §2) · **S15.3 MPIN cooldown / disabled** — 5 free attempts, then 30 s · 1 min · 5 min · 15 min · 1 h with a countdown row; disabled after 10 → OTP + biometric (ADR 2026-09-05d §5; the copy keeps the canvas's *"a code, not a lockout"* tone) · **biometric re-enrolment** — *"A new face or fingerprint was added to this phone — enter your PIN once"* (ADR 2026-09-05d §4) · **foreground inactivity lock** after 5 min idle (ADR 2026-09-05 §7), **suppressed while a draft has digits typed**; the draft returns to the same field after unlock. ⟦tests: C-05a-7⟧
...
> **ADR 2026-09-25 §3** — inputs are PDF (a locked PDF asks for its password once; never stored or sent), CSV, XLS and a **photo or screenshot** read by on-device text recognition; all feed the same mapping, balance check and S7.4 preview. No per-bank parsers, no AI and no server in this phase. ⟦tests: F1-25-2 @M10, F1-25-3 @M10⟧
...
- **Invite:** phone → per-book role + limit grid → **designation (optional)** — picked from the 01 §2 tables for the tenant type, or typed free; one muted line states it plainly: *"A name, not a permission — what they can do is set above"* (06 §1.0 🔒, Option B) → **Send invite** creates it and opens the share sheet; the inviter sends the link from their own phone, the server sends nothing (ADR 2026-09-25 §2). Invite card shows state machine (06 §7): `Invited (expires in 7 d)` → `Joined — verification needed` → `Active` → or `Expired` (one-tap re-invite). **A second admin's screen cannot show whom was invited** — only the inviting device holds the contact (ADR 2026-09-05c §4); the row reads *"Invited by Amrit · awaiting join"* (ADR 2026-09-05f §G).
...
- **Ceremony screens** (04 §6): *Show my code* (huge QR; the 8-digit code beneath rendered as **eight separate character boxes with a visible expiry countdown** 🔒 owner-approved — presentation only, never a password. **Since ADR 2026-09-13d §1/§5 🔒 (ratified 13 Sep 2026) the boxes stand empty, under *Waiting for {name} to enter the code*, until the verifier has begun** — the digits are a commitment-based SAS over both sides' contributions and *cannot exist* before the verifier's arrives. The old derivation from the key fingerprint + the server's invite nonce is retired: a substituting relay could pre-compute it. The invite nonce survives in the QR payload only) ⟦tests: F1-13d-1, F1-13d-2⟧ and *Verify member* (camera default, `Enter code instead`). Success = green tick + confetti-restraint; **mismatch = full red screen, no override**, *Contact support*, event logged. Remote mode (admin toggles per invite): verifier screen instructs *"Call them and ask them to read the code aloud"* — no share button, by design. Delegated: any active member's *Verify member* works.
...
- Wizard = 02 §8 steps verbatim, one screen each: cash count (big keypad, difference shown in words: *"₹230 less than the book — adjust?"*), bank confirm (per account: `Matches ✓ / Doesn't match ▸ reconcile`), tray clearing (pendings, Suspense, aged advances warn-only), confirm & lock (shows the balance vector summary; other devices verify in background; mismatch → both devices alert with a *Recompute* action; **a reader on an older projector shows *"Update the app to verify this close"* and keeps the close as *unverified-by-you*** — the third outcome, ADR 2026-09-05c §3, ADR 2026-09-05f §B).
...
> **ADR 2026-09-25 §7** — the receipt card (S4.2) moves to the **next phase**, where receipts follow the Income Tax department's standard format and this paragraph's *no tax language* line is reopened with that format in hand. ⟦tests: n/a — deferral⟧
...
Language (per member) · **Appearance** (system · light · dark — ADR 2026-09-05f §H12) · book management (create/rename/archive, FY start) → **Your books → Opening balances** (the correction-wizard door, ADR 2026-09-03b ruling 2) · category tree editor · notification preferences · Auto-lock (both values, §15) · export everything (06 §9.2 — always available, plan-independent) · subscription (§20) · about/support (email — ADR 2026-09-25 §4).
...
- Punjabi & Hindi: every screen renders Gurmukhi and Devanagari correctly incl. numerals context; no string overflows in either on smallest supported device (iPhone SE 3rd gen, 375×667) **or the Android floor (360×800, from M12)**.
- **Suite split (ADR 2026-09-05i §8):** the items above run as **F2** (device lab, release candidate); widget-level checks run as **F1** on every push. F2 also covers: screenshot and recording blocked on Android, privacy cover drops under iOS capture, reduced-motion (no shimmer, jump-cut), dark mode parity, and the stopwatch **including** the biometric unlock.
...
- Privacy cover appears in the app switcher; a capture attempt shows the consequence line; Show my code is covered too (ADR 2026-09-05f §G).
...
## 22. Help & diagnostics (S17.x) 🔒 (ADR 2026-09-05f §F) ⟦tests: F1-07-382, F1-07-383, F1-07-384, F1-07-385, F1-07-386, F1-07-387, F1-07-388, F1-07-389, F1-07-390, F1-07-391, F1-07-392, F1-07-393, F1-07-394, F1-07-395, F1-07-396, F1-07-397, F1-07-398, F1-07-399, F1-07-400, F1-07-401, F1-07-402, F1-07-403, F1-07-404, F1-07-405, F1-07-406, F1-07-407, F1-07-408, F1-07-409, F1-07-410, F1-07-411, F1-07-412, F1-07-413, F1-07-414, F1-07-415⟧
**S17 Help** (searchable FAQ hub) · **S17.2 FAQ article** · **S17.3 Contact support** — **email** (`support@rukkafolio.com`) for the pilot; an **in-app AI chat** becomes primary before launch, with email as the hand-off (ADR 2026-09-25 §4); **states what support cannot do** (read a book, recover a key, add a member, bypass a ceremony — 06 §8) · **S17.4 Send diagnostics** — user-triggered, financial values scrubbed, the payload shown before sending.
