...
| M2 | Local persistence & projections | 03 §3: Drift + SQLCipher, projector (pure), balances/snapshots, Recompute determinism; **`held` state for dangling refs, author-gap tracking, close blocked on gaps (ADR 2026-09-05b §3–4); `projectorVersion` + Recompute-on-upgrade, `quick_check` + corruption paths, backup exclusion (ADR 2026-09-05c §3, §6, §8)** | Suite E (client half) |
...
| M4 | Server + sync engine | 03 §2 migrations + RLS, push/pull with `seq` cursors, meta/key channel, outbox (05); **signed records applied to rows, `store_epoch`, `observed` read-your-writes, `seq` revocation cut-off, rate limits + quotas, `maintenance` deletion role (ADR 2026-09-05b); **India region, PITR + snapshot policy, first restore drill, `blob_hash` + shape checks, `phone_ct`/`phone_hmac` + KMS, `SET LOCAL` claims, private bucket + orphan sweep (ADR 2026-09-05c)** | Suites D, E (server half) |
...
| M6 | Auth & devices | 06: OTP, device keys, sessions, linked-devices screen, recovery sheet + nag, min-version gate; **certified-only RLS + server cert verify, 24 h recovery/support windows with Cancel, biometric-set binding, MPIN lockout, new-device notice on every path, signed device/verification records (ADR 2026-09-05d)** | Suite C |
