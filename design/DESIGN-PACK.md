...
**Light:** bg `#F5F0E4` · surface `#FBF8F0` · text `#1A1A18` · muted `#696558` · hairline `#E3DCCB` · primary `#2B3A67` · accent `#C1502E` · money-in `#2B724F` · money-out `#A83232` · pending `#8F5F00` · locked `#8A857A` *(muted and money-in darkened 13 Sep 2026 for AA on `sunk` / `danger-surface` — ADR 2026-09-13 §2 🔒)* ⟦tests: F1-10-12⟧
...
**Type:** Mukta (Latin/Devanagari) + Mukta Mahee (Gurmukhi). display 40/600 · page 28/600 · section 20/500 · body 16/400 · row 14/400 · caption 12/400 · hero amount 44/600 — **sizes come from `design/tokens/tokens.json`; do not invent 11.5/14.5/15.5 (ADR 2026-09-05f §H4)**; line-height per role, never below 1.4 for Gurmukhi/Devanagari. **All amounts tabular figures, ₹ prefix, Indian grouping (₹1,24,500) — no paise on screen; Latin digits in every amount column.** **Status colours** (`success`/`warning`/`info`/`danger`) exist for icons, borders and words — **never for amounts**; the palette above is the generated canvas band, not a second source (ADR 2026-09-05f §H1, §H5).
...
## S1 — Home
Design the home screen, top to bottom: top bar with a scope chip (pill, book name + chevron) and sync/notification icons right. Then **hero**: small muted caps label "TOTAL MONEY YOU HAVE", a 34px bold tabular amount, and a muted breakdown line beneath ("Indian Bank ₹47,000 · Cash ₹14,500"). Then a **books-balanced card** — check icon and "Books balanced" in the `success` token (icon and word, never an amount), chevron right, and beneath in muted text "difference nil" — Dr/Cr totals live on the trial balance, not on Home (ADR 2026-09-05f §F). While a book rebuilds, this card is replaced by **S1.4** — the 2px determinate loader rule with "1,240 of 3,890 entries restored". In Everything scope a book card with an author gap carries a small **provisional** badge. Then a **position card** (P2) with rows: You will get (green) · You will give (red) · Advance out (with the holder's name as meta). Then a slim **month card**: muted "This month" left, "In ₹1,75,000 · Out ₹1,38,200" right in green/red. Then a "Today · 30 Aug ▸ day book" label and 3 entry rows. Then, **docked directly above the bottom navigation**, a 4-across grid of verb buttons — Money in · Money out · Gave on credit · Took on credit — each a bordered tile with an icon above a 2-line-safe label. **The verb buttons carry no figures.** Bottom nav: **Home · Ledger · ( + ) · Inbox (badge) · Menu** — four tabs plus a docked centre action that is not a tab (no active state, no label); the same bar on every screen (design-system §4.1, 13 §3.1, ADR 2026-09-05f §A). **No bell icon** in the top bar — Inbox is the one tray.
...
## S15 — App lock
The screen between the app icon and the books. Centred: the Rukka Folio mark, then "Unlock to open your books". The biometric prompt **fires automatically on arrival** — the user should usually never touch this screen. Beneath, a quiet text button **Use PIN instead**. Design three states: waiting for Face ID · failed, showing "Try again" plus the PIN option (the mark stays **sealed** and shakes ±2 units, 3 cycles, 240ms — the seal never breaks on the prompt, 11 §4.5) · PIN entry, as six grouped boxes with a quiet **Forgot PIN** link (OTP + biometric — the device passcode is never offered, 06 §4.4 / ADR 2026-09-01). Nothing else on the screen — no balances, no book names, no branding beyond the mark. **Also design (ADR 2026-09-05f §B):** the **biometric re-enrolment** variant — one line, "A new face or fingerprint was added to this phone — enter your PIN once"; and **S15.4 Device suspended** — the app open in read-only with a persistent top banner ("This phone was signed out by the server. Your books are safe and readable; entering is paused until we can confirm.") and a *Retry* action — never a hard lock, nothing wiped.
...
## O0 · Splash
The sealed mark centred on a plain paper field with the **stacked lockup** beneath — mark above "Rukka Folio", *Folio* at weight 300 — and nothing else: no tagline, no spinner, no version (11 §4.5). Dark: ink field, paper-book inverse mark. The **sealed→open animation plays only when the session is already open** (m 1.0, once, hold into Home); on a cold start that ends at the lock screen nothing moves — the seal breaks only on the successful unlock (m 0.6). Design the **slow-start** variant too: past 3s the 2px loader rule fades in beneath the lockup with "Opening your books." — never a spinner.
...
## S15.3 · Enter PIN
Six boxes, a **Face ID** button above them for returning to the fast path, and a quiet **Forgot PIN** link. Design the wrong-PIN state (attempts remaining) and the forgot flow's first screen: "We'll send a code to your number, then you can set a new one — your books are not affected." **Keep the tone — "a code, not a lockout" — but add the behaviour (ADR 2026-09-05d §5, 05f §B):** a **cooldown** row with a countdown after the fifth wrong try (30 s → 1 min → 5 min → 15 min → 1 h), and a **PIN disabled** state after ten — "Too many tries. We'll send a code to your number and you can set a new PIN." (OTP + Face ID).
...
**R2.0 · Silent restore (the common case).** Before any fork appears: if the platform key store returns the key (same Apple ID), show a brief calm screen — a tick, "Your books are back", the number of books restored — then straight to Home. No choices, no explanation of cryptography. Most people will only ever see this one. ⟦tests: F1-07-279, F1-07-280, F1-07-281, F1-07-282, F1-07-283⟧

**R2.1 · After OTP on a new phone — the fork.** A quiet screen: "Let's get your books back on this phone", then three options as full-width rows, in this order, each with a one-line explanation: **Use another phone you're signed in on** · **Ask your trusted members** · **Use your recovery sheet** (with a sub-line *paper or the file you saved*). A muted line at the bottom: "Your family books can also be restored by your family — your own private book cannot." ⟦tests: F1-07-272, F1-07-273, F1-07-274, F1-07-275, F1-07-276, F1-07-277, F1-07-278, F1-07-416, F1-07-417, F1-07-418, F1-07-419, F1-07-420, F1-07-421, F1-07-422⟧

**R2.2 · Ask your trusted members (S11.2).** The live waiting screen. Heading "Ask any 2 of 3 to approve", then a row per guardian with avatar, name, and a state: **approved ✓** (green) · **waiting…** (muted, with a short inline **loader rule** — 2px hairline + ink segment, 11 §4.5; never a spinner) · **not asked**. Each waiting row has a small **Call** link — the pack's advice is literally "phone them, they are expecting this". A progress line: "1 of 2 approvals". Design the **completed** state too: all ticks, then a single calm line with a count — "1,240 of 3,890 entries restored" — over the determinate loader rule (counts, never percentages). This screen may be open for minutes; it must not feel stuck.
...
**R2.5 · Nothing worked yet.** 🔒 **Build the whole screen around the word *yet*** — three bordered rows in green / amber / grey: family books restorable now (carrying the button), the private book *"still sealed on our server, exactly as you left it"* with the two keys that will open it *"today, next week, or next year"*, and the readable copy as a way to restart from closing balances. No "destroyed", no "contact support", no retry. Reached only when every rung has failed. Honest but not falsely final — the entries still exist on the server, sealed. Heading: **"We can't open your private book on this phone yet."** Then three plain rows: ⟦tests: F1-07-284, F1-07-285, F1-07-286, F1-07-287, F1-07-288, F1-07-289⟧
...
One primary button: **Continue and set up this phone**. 🔒 Do **not** write that the data is destroyed — it is not — and do not soften into "contact support", which cannot help. No retry loop. ⟦tests: F1-07-286, F1-07-287⟧

## R4 · Backup settings (S11.4) *(the same controls as onboarding O5, revisited later)*
🔒 **Repeat O5's wording and ordering verbatim** — add only the per-row status: *on* · *last saved 12 Apr* · an amber **never saved** badge on the readable copy, which is the one row whose state actually matters. ⟦tests: F1-06-12⟧
...
**S13 · Settings.** Language (three options, changes immediately) · **Appearance** (system · light · dark) · notification preferences by type · **Auto-lock** showing both values (background 2 min · idle 5 min) · **Personal Book lock** · **Your books → Opening balances** (the correction-wizard door, ADR 2026-09-03b ruling 2) · **Export everything** · date format. Plain grouped rows, no cleverness (ADR 2026-09-05f §F, §H12).
...
## Loading states (ADR 2026-09-03d, 05f §H6)
Every list screen (S1 today, S3, S4, S6, S7.1, S8.1, S9, S11) gets a **skeleton** variant — static ruled rows at the true pitch using the `skeleton-label` / `skeleton-amount` tokens, no shimmer, one screenful. The **loader rule** (2px `loader-track` + `loader-segment`, never `--hair`/`--text`) is drawn on O0 slow start, R2.0, R2.2 and S16.2 with its determinate copy "{done} of {total} {things} {verbed}", and on **S1.4 Book incomplete — rebuilding**.

## States from the 5 Sep ADRs (05f §B, §D)
Design as components, then place: **banner** (persistent — device suspended S15.4, read-only S12.5, book full) · **cancel-window card with countdown** ("Recovery in progress · 23 h 40 m · Cancel" S11.9; "Support action pending · Cancel" S11.10) · **provisional badge** on P2 cards and the status chip ("Waiting for entries from Ramesh's phone") · **held row state** in P1 ("waiting for the entry this changes") · **S10.5 Close blocked — waiting on a device** · **"Update the app to verify this close"** on S10.2/S10.4 · **S19.5 This phone has been modified** (a notice, never a wall) · Inbox card types: book full, write lost, key wait, author gap, quarantine, device added, integrity.

---

## Deliverables checklist
Components for P1/P2/P3 and all atoms with every state · every screen above in **light and dark** — dark designed, not a token swap over light markup (no `#d9d5cd` frames, no `#ffffff`, no template orange, no rgba shadows — ADR 2026-09-05f §H7) · every screen in **all three languages** · empty/error/offline **and loading** variants for each list · tappable prototypes of **four** flows: add a cash entry, review a flagged entry, **close a month (F9), recover on a new phone (F11)** · redlines referencing token names, never raw hex · every screen holds at 375×667 **and 360×800** at 200% font scale.
