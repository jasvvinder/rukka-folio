...
## 2. The two new tokens — approved 5 Sep 2026 (ADR 2026-09-05f §H11); the status family landed 12 Sep 2026 ⚠️ awaiting owner sign-off

`pending`, `locked`, `sunk`, `scrim` are **approved** at the values below/in tokens.json; `sunk` carries the caveat that light `credit` is darkened one step so amounts pass AA on it (§3.1). **Status family — values landed 12 Sep 2026, ⚠️ owner sign-off outstanding** (the 5 Sep pattern: proposed in code, ratified by the owner): light `success` #276A49 · `warning` #7C5200 · `info` #1F6785 · `danger` #9C2C2C · `on-danger` #F5F0E4 · `focus-on-primary` #F5F0E4; dark `success` #5CB489 · `warning` #E0AE55 · `info` #86C6DC · `danger` #E08C8C · `on-danger` #1A1A18 · `focus-on-primary` #1A1A18. Every one is measured by `scripts/check_contrast.dart` on all four grounds in both modes (110 gated pairs pass). `danger` is deliberately **not** `debit`, so a security warning never reads as money out; `info` is deliberately **not** `primary`, separated by hue. Still ⚠️ pending a session: `minTouchTarget: 44`, a dark reduced-motion track, a dark `shadow-soft`, and the light `credit` darkening §3.1 ruled.
