...
11. **No spinner, anywhere 🔒 (owner-ruled 3 Sep 2026, ADR 2026-09-03d).** The loader is a *rule*: 2px `loader-track` hairline + `loader-segment` ink — never indigo, never bahi, never the mark. Determinate with a count whenever the app can count ("1,240 of 3,890 entries restored"), indeterminate only with a verb, shown after 200ms, escalates with one status line at 8s, announced as a live region. (11 §4.5) ⟦tests: F1-13-8⟧
12. **Skeletons are static ruled rows** at the true pitch — no shimmer, zero layout shift, one screenful max. (11 §4.5)

## 2. The two new tokens — approved 5 Sep 2026 (ADR 2026-09-05f §H11); the status family **signed off 13 Sep 2026** (ADR 2026-09-13 §2)

`pending`, `locked`, `sunk`, `scrim` are **approved** at the values below/in tokens.json. **Status family — ratified by the owner 13 Sep 2026** (ADR 2026-09-13 §2 🔒 ⟦tests: F1-10-12⟧, closing the sign-off the 12 Sep token session opened; the 5 Sep pattern is proposed in code, ratified by the owner): light `success` #276A49 · `warning` #7C5200 · `info` #1F6785 · `danger` #9C2C2C · `on-danger` #F5F0E4 · `focus-on-primary` #F5F0E4; dark `success` #5CB489 · `warning` #E0AE55 · `info` #86C6DC · `danger` #E08C8C · `on-danger` #1A1A18 · `focus-on-primary` #1A1A18. Every one is measured by `scripts/check_contrast.dart` on all four grounds in both modes (**110 gated pairs pass, 0 waived pending ruling** — the audit is clean for the first time). `danger` is deliberately **not** `debit`, so a security warning never reads as money out; `info` is deliberately **not** `primary`, separated by hue. **No `PROPOSED` marker remains in tokens.json**; the four that were left there after the 5 Sep approval (light and dark `sunk`, `pending`, `locked`, `scrim`) were stale and are gone. Still ⚠️ pending a session: `minTouchTarget: 44`, a dark reduced-motion track, and a dark `shadow-soft`.
...
Computed (WCAG 2.1 relative luminance), on light `bg #F5F0E4`: `text` 15.3:1 AAA · `primary` 9.7:1 AAA · `debit` 5.8:1 AA · `text-muted` 5.1:1 AA (darkened 13 Sep, was 4.8) · `credit` 5.1:1 AA (darkened 13 Sep, was 4.6) · `pending` 4.9:1 AA (darkened from the first proposal, which measured 4.0) · `accent` 4.1:1 — **below AA by brand-fixed value: use for the seal, icons, and large text only, never body text** · `locked` 3.2:1 — **intentionally sub-AA: marks disabled content, never the only signal (🔒 icon always accompanies)**. Dark set on `#1A1A18`: text 15.3 · pending 8.0 · primary 7.1 · text-muted 6.7 · credit 6.1 · accent 5.5 · debit 4.8 — all amount/text roles ≥ AA. The audit script becomes **`scripts/check_contrast.dart`, run by `ci.sh`**, computing every text/amount role on **all four grounds** (`bg`, `surface`, `sunk`, `danger-surface`), both modes (ADR 2026-09-05f §H2, §H10); this prose is superseded by its output. Grayscale check (07 §18) still applies — ~1 in 12 male users is red-green deficient.

## 3.1 Accessibility conformance 🔒 (target: WCAG 2.2 AA) ⟦tests: F1-10-12, F1-10-13, F1-10-14, F1-10-15, F1-10-9, F1-10-10, F1-10-11⟧

**Contrast — computed, both themes, on `bg` *and* `surface`.** All text and amount roles pass AA. Two deliberate exceptions, each safe because it never carries meaning alone: `accent` (light 4.15:1) is restricted to the seal, icons and large text; `locked` (3.23 / 3.80) marks disabled content and is always accompanied by a 🔒 icon. `debit` dark had been lifted to `#D4776F` after measuring 4.29:1 on `surface` at its previous value. **4 Sep 2026 (owner-ruled, ADR 2026-09-03d):** dark `credit`/`debit` moved to the motion page's hues: `#4FA37A` / `#CB6F6F`. The page's original debit `#C96A6A` measured 4.31:1 on `surface`, below AA for 14–16px amounts on cards, so debit was lifted one step (owner-approved). Measured now: credit 5.69:1 on `bg`, 5.13:1 on `surface`; debit 5.02:1 on `bg`, 4.53:1 on `surface` — all AA ✓. **Re-run the audit on any token change; contrast must be checked against all four grounds — `bg`, `surface`, `sunk` and `danger-surface`.** Measured 5 Sep 2026 (ADR 2026-09-05f §H2): light `credit` on `sunk` **4.30:1** and on `danger-surface` **4.20:1**, dark `debit` on `danger-surface` **4.04:1** — all below AA. Ruling: light `credit` is **darkened one step**; amounts are **never placed on `danger-surface`**. **Hexes landed 13 Sep 2026 (ADR 2026-09-13 §2 🔒), and the audit is now clean — 110 gated pairs pass, 0 waived pending ruling:** light `credit` **#2F7A55 → #2B724F** (on `sunk` 4.30 → **4.79**) and light `text-muted` **#6E6A5E → #696558** (on `sunk` 4.46 → **4.81**, on `danger-surface` 4.36 → **4.70**), the latter closing the two unruled pairs `check_contrast` found on 7 Sep. Two things about the credit step are worth keeping: the *full* credit→success step lands exactly on `success` #276A49, so the hex is **half** that step — and the resulting closeness is safe precisely because `credit` is numerals-only and `success` is a status word plus an icon, so the two are never read against each other. `text-muted` was darkened rather than the alternative the finding offered (*keep captions off `sunk` and `danger-surface`*): a placement rule for captions is unenforceable in code, where a token value is checked on every run. ⟦tests: F1-10-12, F1-10-13, F1-10-14, F1-10-15⟧ **Focus** (WCAG 2.2 §2.4.11 / §1.4.11): the ring is `focus` on every ground and **`focus-on-primary`** on primary buttons — today's `focus` equals `primary` and vanishes there. **Landed 12 Sep 2026:** `focus-on-primary` is the paper/ink inverse of each mode (light `#F5F0E4` at 9.70:1 on `primary`, dark `#1A1A18` at 7.13:1) — the maximally visible ring rather than a fourth hue. **Finding, recorded rather than designed away (12 Sep 2026):** the four status colours are near-iso-luminant by construction — light relative luminance 0.091–0.117, dark `success` 0.367 vs `danger` 0.365 — so in **grayscale they are not distinguishable from each other**. Chasing luminance separation would have distorted a palette that is otherwise brand-correct, and it is unnecessary: 07 §1 rule 3 and the grayscale test (07 §18) are satisfied by the icon and the word, which every status surface already carries. The consequence is a hard constraint on future use — **a status colour may never be the only signal**, and no surface may encode meaning in the tint alone.

**Beyond contrast — the rules that make this app usable non-visually:**
1. **Language of parts (WCAG 3.1.2) 🔒** — account names, notes and party names are user-typed and may be in any of the three scripts *inside* a UI running in another language. Every such string carries its own `lang` attribute (`pa`, `hi`, `en`) so VoiceOver switches voice instead of reading Gurmukhi with an English synthesiser. This is the single most important a11y rule in a trilingual product and it must be implemented at the text-widget level, not per screen. ⟦tests: F2-13-1 @M12⟧
2. **No timing traps (WCAG 2.2.1) 🔒** — the 10-second entry Undo is a *convenience*, never the only correction path: the entry remains amendable afterwards (02 §5), and the toast is dismissible rather than auto-only. OTP resend countdowns are informational and extendable. Nothing the user must act on expires without a way back. ⟦tests: F1-07-17⟧
3. **Status messages (WCAG 4.1.3) 🔒** — save toasts, sync-state chips, review-flag changes and quorum progress announce politely (`aria-live="polite"` / `LiveRegion`) without stealing focus. ⟦tests: F2-13-2 @M12⟧
4. **Table semantics (WCAG 1.3.1) 🔒** — the A/C statement is a real data table: column headers associated with cells, and each amount announced with its column and side ("two thousand four hundred rupees, credit, balance one thousand three hundred seventy credit"), never as a bare number. ⟦tests: F2-13-3 @M12⟧
5. **Amount announcements 🔒** — every amount reads as value **plus direction** ("₹2,400, money out"), so the colour's meaning survives with the screen reader on. ⟦tests: F2-13-4 @M12⟧
...
7. **Camera-free path 🔒** — the verification ceremony's QR always has the 8-digit code as an equal alternative, so a blind user or a broken camera never blocks joining. ⟦tests: F2-13-5 @M12⟧
...
- [ ] ⚠️ Token session (ADR 2026-09-05f §H) — **status family + `focus-on-primary` landed 12 Sep 2026, owner sign-off outstanding**; still open: light `credit` darkened for `sunk` · dark reduced-motion track · dark `shadow-soft` · `minTouchTarget` · per-role line-heights (Indic floor 1.4) · canvas type roles · `iconography` gains nav stroke 1.8 and sizes 21/34/36 · `$meta.version` bump
...
- [ ] Components for the 5 Sep states: banner (suspended / read-only / book full) — **built in code 12 Sep as `RkRestrictionBanner` (`app/lib/shared/widgets/rk_restriction.dart`), still undrawn on canvas; reconcile against DESIGN-PACK §11 S12.5's "persistent slim banner, not a modal"**, toast, countdown (cancel windows, PIN cooldown), progress meter, provisional badge, held row state, S15.3 cooldown + disabled
